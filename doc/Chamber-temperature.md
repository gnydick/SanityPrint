SanityPrint use `M141/M191` command to control active chamber heater.  

If `Activate temperature control` is checked, SanityPrint will insert `M191` command at the beginning of the gcode(before `Machine G-code`).  
![image](./images/activate_chamber_heater.jpg)  
*Note: If the machine is equipped with an auxiliary fan, SanityPrint will automatically activate the fan during the heating period to help circulate air in the chamber.*  
