...
void MeshBooleanUI::render_content(GLCanvas3D& parent, ImGuiWrapper* imgui, bool is_dark_mode)
{
    ...
    // Extra top padding so the operation tabs sit further from the popup top border.
    ImGui::Dummy(ImVec2(0.0f, ImGui::GetStyle().WindowPadding.y * 0.1f));
...
bool MeshBooleanUI::draw_operation_mode_tabs()
{
    ...
    // Make tab icons slightly smaller than list icons.
    const float tab_icon_size = m_computed_icon_size_display * 0.9f;
    const float font_height = ImGui::GetFontSize();
    const float frame_padding_y = ImGui::GetStyle().FramePadding.y;
    const float tab_height = std::max(font_height, tab_icon_size) + 3.5f * frame_padding_y;

    // Compute max tab length to keep three tabs visually aligned
    const float base_tab_width = 4 * ImGui::GetStyle().FramePadding.x + tab_icon_size + MeshBooleanConfig::ICON_SPACING + std::max(ImGui::CalcTextSize(_u8L("Union").c_str()).x,
    ...
    // Wider + shorter tabs.
    const float tab_width = base_tab_width * 1.2f;

    // Calculate tab strip total width and store dynamic widths
    float total_width = 3 * tab_width;
    ...
    ImGui::BeginGroup();
    if (draw_tab_button("union", _u8L("Union").c_str(), m_operation_mode == MeshBooleanOperation::Union, ImVec2(tab_width, tab_height), true, 0)) {
        m_operation_mode = MeshBooleanOperation::Union;
        clicked_any = true;
        if (on_apply_color_overrides) on_apply_color_overrides(m_operation_mode);
    }
    ImGui::SameLine(0, 0);
    if (draw_tab_button("intersection", _u8L("Intersection").c_str(), m_operation_mode == MeshBooleanOperation::Intersection, ImVec2(tab_width, tab_height), true, 1)) {
        m_operation_mode = MeshBooleanOperation::Intersection;
        clicked_any = true;
        if (on_apply_color_overrides) on_apply_color_overrides(m_operation_mode);
    }
    ImGui::SameLine(0, 0);
    if (draw_tab_button("difference", _u8L("Difference").c_str(), m_operation_mode == MeshBooleanOperation::Difference, ImVec2(tab_width, tab_height), true, 2)) {
...
void MeshBooleanUI::draw_action_buttons()
{
    ...
    // OK button (green)
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.082353f, 0.752941f, 0.349020f, 1.00f));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.00f, 0.6f, 0.20f, 1.00f));
...
void MeshBooleanUI::draw_only_entity_checkbox()
{
    ...
    // Align with list width so the checkbox sits closer to the list frame.
                                   ...
                                   const ImVec2& size, bool enabled, int border_type)
{
    ...
    const float icon_size = m_computed_icon_size_display * 0.9f;
    ...
    // Snap to pixel grid to reduce blur/halo from linear sampling at small sizes.
    icon_pos.x = IM_ROUND(icon_pos.x);
    icon_pos.y = IM_ROUND(icon_pos.y);
    ImVec2 icon_max = ImVec2(IM_ROUND(icon_pos.x + icon_size), IM_ROUND(icon_pos.y + icon_size));
    if (icon_id) draw_list->AddImage(icon_id, icon_pos, icon_max);
                                    ...
                                    ImU32 title_bg_color)
{
    ...
    // Slightly shorter title bar for a tighter layout.
    const float title_h = m_computed_list_title_height * 0.8f;
    ImVec2 title_max = ImVec2(pos.x + size.x, pos.y + title_h);
    ...
    if (m_check_list_icon_id && !items.empty()) {
        // Make the select-all icon a bit smaller than normal list icons.
        float button_size = m_computed_icon_size_display * 0.9f;
        float button_margin_y = (title_h - button_size) * 0.5f;
    ...
    ImVec2 start_pos = ImVec2(
        ...
        title_min.y + (title_h - title_text_size.y) * 0.5f
    );
    ...
    // Vertically center the count inside the title bar (match the main title centering).
    count_pos.y = start_pos.y + (title_text_size.y - count_text_size_small.y) * 0.5f;
    ...
    float available_height = size.y - title_h - title_to_list_spacing;
...
void MeshBooleanUI::set_centered_cursor_x(float width) const
{
    ...
    float min_expected_width = m_computed_list_width;
                              ...
                              std::vector<std::vector<unsigned int>>& out_groups)
{
    ...
    if (m_target_mode == BooleanTargetMode::Object) {
        ...
        for (int obj_idx : obj_list) {
            ...
            // Explicit construction to avoid MSVC emplace_back deduction issues
            out_items.push_back(ListItemInfo(obj_name, ModelVolumeType::MODEL_PART, true, obj_idx));
        ...
        for (unsigned int idx : list) {
            ...
            if (!mv) continue;
            // Explicit construction to avoid MSVC emplace_back deduction issues
            out_items.push_back(ListItemInfo(mv->name, mv->type(), false));
...
void MeshBooleanUI::draw_warnings()
{
    ...
    if (!current_warnings.empty()) {
        ...
        auto hints = m_warning_manager->get_inline_hints_for_state(m_operation_mode, m_target_mode, *m_volume_manager);
        if (!hints.empty()) {
