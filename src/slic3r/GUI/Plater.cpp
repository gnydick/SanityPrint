...
Sidebar::Sidebar(Plater* parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent)), m_last_em_unit(wxGetApp().em_unit())
{
...

    {
        ...
        // p->pSearchBk->SetBorderColor(StateColor(std::make_pair(0xDBDBDB, (int)StateColor::Disabled), std::make_pair(0x2E86C1,
        ...
        //   p->m_search_bar->SetForegroundColour("#2E86C1");
...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
                          _L("There are some unknown filaments mapped to generic preset. Please update Sanity Print or restart Creality "
        ...
        try {
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    // BBS: version check
                    ParsedCrealityVersion parsed_app_version = parse_creality_version(std::string(SANITYPRINT_VERSION), parse_build_id_string(std::string(SLIC3R_BUILD_ID)));
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by SanityPrint, load geometry data only."), _L("Load 3mf"));
#endif
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Sanity Print, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by SanityPrint, load geometry data only."), _L("Load 3mf"));
                    //     }
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    // update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - SanityPrint");
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg  = _L("Importing to Sanity Print failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8(
            "\n" + _L("Tips:") + "\n" +
            _L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Sanity Print(windows) or CAD softwares."));
    }
#endif // APPLE & LINUX
