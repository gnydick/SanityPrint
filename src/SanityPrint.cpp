...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in Sanity Print before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in Sanity Print before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Sanity Print before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in Sanity Print before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in Sanity Print before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in Sanity Print before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Sanity Print before uploading."},
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on Sanity Print before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Sanity Print."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    set_current_thread_name("sanityprint_main");
    ...
    /*#if !defined(_WIN32) && !defined(__APPLE__)
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch SanityPrint GUI soon";
        //
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current SanityPrint Version %1%")%SLIC3R_VERSION;
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into SanityPrint process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a SanityPrint process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the SanityPrint process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"SanityPrint is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using SanityPrint.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes SanityPrint "
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: SanityPrint [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
...
//   e.g.  SanityPrint_Slicer.DLL + 0x00123456  ->  look up RVA 0x00123456 in PDB
    ...
    uintptr_t      pReserved)
{
    ...
    lstrcatW(logPath, L"sanityprint_invalid_param_stack.log");
...
extern "C" {
    __declspec(dllexport) int __stdcall sanityprint_main(int argc, wchar_t **argv)
    {
        BOOST_LOG_TRIVIAL(error) << "sanityprint_main start";
...
//BBS: register default exception handler
        ...
        auto dmpCallBack = [](const wchar_t* dump_path,
            ...
            if (succeeded) {
                ...
                std::string processNameStr = timeStr + std::string("_") +  SLIC3R_PROCESS_NAME + std::string("_") + SANITYPRINT_VERSION + std::string("_") + PROJECT_VERSION_EXTRA + std::string(".dmp");
