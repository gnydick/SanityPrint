...
#define CREALITY_TAG_60 "generated by SanityPrint"
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: SanityPrint is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::SanityPrint, SLIC3R_APP_NAME },
    { EProducer::SanityPrint, CREALITY_TAG_60 },
    { EProducer::SanityPrint, CREALITY_TAG_51},
    { EProducer::SanityPrint, CREALITY_TAG_CLOUD},
    { EProducer::SanityPrint, BAMBU_TAG},
    { EProducer::SanityPrint, BAMBU_TAG_2},
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by SanityPrint,
        // extract the config from it
        if (m_producer == EProducer::SanityPrint || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from SanityPrint's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::SanityPrint) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::SanityPrint: { return process_bambuslicer_tags(comment); }
...
bool GCodeProcessor::detect_producer(const std::string_view comment)
{
    for (const auto& [id, search_string] : Producers) {
        ...
        if (pos != comment.npos) {
            ...
            if (m_producer == EProducer::SanityPrint) {
