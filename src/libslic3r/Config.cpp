...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or SanityPrint header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; SanityPrint";

        std::string sanityprint_gcode_header = std::string("; generated by ");
        sanityprint_gcode_header += SLIC3R_APP_NAME;

        // for version 5.1
        const std::string SLIC3R_APP_NAME_1            = "Creality_Print";
        std::string sanityprint_gcode_header_1 = std::string("; generated by ");
        sanityprint_gcode_header_1 += SLIC3R_APP_NAME_1;

        // for Creality Cloud Slicer
        const std::string CLOUD_SLIC3R_APP_NAME            = "Creality_Cloud_Slicer";
        std::string sanityprint_cloud_gcode_header = std::string("; generated by ");
        sanityprint_cloud_gcode_header += CLOUD_SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(sanityprint_gcode_header_1.c_str(), line_c, strlen(sanityprint_gcode_header_1.c_str())) == 0 ||
                strncmp(sanityprint_gcode_header.c_str(), line_c, strlen(sanityprint_gcode_header.c_str())) == 0 ||
                strncmp(sanityprint_cloud_gcode_header.c_str(), line_c, strlen(sanityprint_cloud_gcode_header.c_str())) == 0
                ) {
                header_found = true;
                break;
            }
